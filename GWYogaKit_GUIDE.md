...
GWYogaKit 是 **Yoga 布局引擎的 Swift/ObjC 封装层**。提供 `UIView.style` 属性读写 flexbox/grid 样式，支持链式调用、CSS 字符串、动画、数据绑定、调试工具等。所有布局由 Yoga 引擎计算，不依赖 Auto Layout。
...
let child = UILabel()
child.text = "Hello"
child.style.width(80).height(40)
    .margin(.right, 8)
    .flexGrow(1)

// 层级
container.addChild(child)  // addSubview + Yoga insertChild
```
...
// 指定角圆角
view.style.cornerRadius(YogaCornerRadii(topLeft: 12, topRight: 12, bottomLeft: 0, bottomRight: 0))
view.style.cornerRadius([.topLeft: 12, .topRight: 12])  // 字典方式
```
...
// CSS 文件加载
view.style.loadStylesheet("styles.css")  // 从 main bundle
view.style.loadStylesheet("styles", bundle: .module)

// 全局默认样式
YogaCSSConfig.registerDefault(stylesheet)
```
...
```swift
// 直接设置 NSAttributedString（自动 isIntrinsic）
view.style.attributedText = NSAttributedString(
    string: "Hello",
    attributes: [.font: UIFont.boldSystemFont(ofSize: 16)]
)

// Builder 便捷构造
view.style.attributedText { builder in
    builder.text("Hello ", font: .boldSystemFont(ofSize: 16))
    builder.text("World", font: .systemFont(ofSize: 16), color: .gray)
}

// 行数控制
view.style.numberOfLines = 2
```
...
// 2. Combine Publisher.bind
let publisher = Just(100 as Float)
let cancel = publisher.bind(to: view.style, keyPath: \.width)

let statePublisher = CurrentValueSubject<CGFloat, Never>(0)
let cancel2 = statePublisher.bind(to: view.style) { style, value in
    style.width = value
}

// 3. 批量合并（同一 RunLoop 多次变更只触发一次 markDirty）
...
UILabel().yog()
    ...
    .numberOfLines(2)
    .width(200).height(50)           // Yoga 属性
    .backgroundColor(.white)         // UIKit 属性
    .cornerRadius(8)                 // 视觉样式
    .flexDirection(.row)             // Yoga 属性
```
...
| `addChild(_:)` | `[view ygk_addChild:child]` | 层级构建 |
| `Container { Text() }` | `YGKVStack { ... }` | DSL（ObjC 用 block） |
...
| `cssID` / `cssClass` | `view.cssTagName` | 命名不同 |
...
### DO
- ✅ 使用 `view.style` 配置布局，提供 vs `view.yoga` 更好的可读性
- ✅ 使用链式 API 在闭包中配置所有属性（`view.style { ... }`）
- ✅ 使用 `addChild(_:)` 替代 `addSubview()` 确保 Yoga 树同步
- ✅ 对于文本/UIImageView 设 `isIntrinsic = true` 自动测量内容
- ✅ 用 `coalesceLayout()` + `YogaBinding` 批量更新多个样式属性
- ✅ CSS 字符串适合动态样式，链式 API 适合静态布局
- ✅ `attributedText` 自动设置 `isIntrinsic`，不用手动处理
- ✅ 用 `debugPrint()` 在开发阶段验证布局树结构

### DON'T
- ❌ 不要混用 `addSubview` 和 `addChild`，优先用 `addChild`
- ❌ 不要在设置多个样式属性时反复触发布局——用 `view.style { ... }` 闭包集中设置，或绑定用 `coalesceLayout()`
- ❌ 不要忘记在 `YogaLayoutView` 添加子视图后调用 `performYogaLayout()`
- ❌ 不要在 ObjC 中使用 Swift-only API（Combine、YogaState 等）
- ❌ 不要在 macOS 上使用 `.style` 别名（用 `.yoga`）
- ❌ `CACornerMask` 指定角需要同时设置 `cornerRadius` + `maskedCorners`
...
// 注意：addChild 自动管理 Yoga node 树，内部操作：
// parent.style.node.insertChild(child.style.node, at: index)
// parent.addSubview(child)
//
// removeFromParent 内部操作：
// child.style.node.owner?.removeChild(child.style.node)
...
3. `YogaLayoutView` 在 `layoutSubviews()` 中自动调用 `performYogaLayout()`
...
*文档版本：2026-05-16 · 与 GWYogaKit 代码同步更新*
